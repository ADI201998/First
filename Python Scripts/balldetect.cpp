...
Mat frame, hsv, mask, gauss, erosion_dst, dilation_dst, mask1, add_im;
...
int main( int argc, char** argv )
{
	...
	while(1)
	{
		...
		imshow("HSV", hsv);
		inRange(hsv, Scalar(20,50,10), Scalar(70,255,255), mask);
		imshow("Range", mask);
    	//Threshold the HSV image to get only white colors
    	inRange(hsv, Scalar(0,0,0), Scalar(0,255,255), mask1);
    	imshow("MASK",mask1);
		...
		Erosion( 0, 0 );
  		...
  		Erosion(0,0);
  		Dilation(0,0);
  		bitwise_and(dilation_dst,mask, add_im);
  		imshow("X",add_im);
		vector<vector<Point> > contours;
		vector<Point> approx;
		vector<Vec4i> hierarchy;
		findContours(add_im, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE, Point(0,0));
		...
		for(int i=0; i<contours.size(); i++)
		{
			...
			//cout<<x;
			e = 0.008*arcLength(contours[i], true);
